...
class Program
{
    ...
    static async Task Main(string[] args)
    {
        ...
        Console.WriteLine("OpenTelemetry instrumentation configured successfully!\n");
        Console.WriteLine("Starting application workflow...\n");

        // Simulate application workflow with tracing
        await RunApplicationWorkflow();

        Console.WriteLine("\nApplication completed. Check the traces and metrics above!");
    }

    private static async Task RunApplicationWorkflow()
    {
        ...
        try
        {
            Console.WriteLine("Step 1: Processing user request...");
            await ProcessUserRequest("user123");

            Console.WriteLine("Step 2: Performing calculation...");
            await PerformCalculation(42);

            Console.WriteLine("Step 3: Simulating HTTP call...");
